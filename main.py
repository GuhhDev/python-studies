...
class TelaEdicao(Screen, Geral):
    ...
    def bt_girar_anti_horario(self):
        ed.spin_image('anti_horario')
        self.exibir_imagem()

    def bt_girar_horario(self):
        ed.spin_image('horario')
        self.exibir_imagem()

    def bt_preto_branco(self):
        ed.remove_color_image()
        self.exibir_imagem()

    def bt_cancelar(self):
        self.mudar_tela('tela_inicial', 'right')

    def bt_salvar(self):
        ed.save(ed.img_local, f'{ed.img_nome}editada')
